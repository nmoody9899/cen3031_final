  ...
  const handleSubmit = (e) => {
    ...
    updateCategory(match.params.slug, { name: name }, user.token)
      .then((res) => {
        ...
        setLoading(false);
        setName("");
        toast.success(`Category "${res.data.name}" udpated successfully`);
        history.push("/admin/category"); //need to redirect here otherwise get error because category no longer exists
      })
      .catch((err) => {
        setLoading(false);
        console.log(err);
